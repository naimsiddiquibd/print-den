...
const PhotoLibrary = () => {
  ...
  const handleFileUpload = (e) => {
    ...
    const fileArray = Array.from(files); // array of raw image files
    setSelectedImg((prevImages) => [...prevImages, ...fileArray]); // Add the new images to the state (raw files)

    const fileArray2 = Array.from(files).map((file) =>
      URL.createObjectURL(file)
    ); // Create object URLs for image preview
    setImages((prevImages) => [...prevImages, ...fileArray2]); // Add the new images to the state (for preview)
  };
  ...
  const handleContinue = () => {
    navigate("/choose-size", {
      state: { selectedImages: selectedImg, prevImages: images },
    }); // Pass images to ChooseSizes route
  };
      ...
      <div className="lg:w-[1180px] lg:mx-auto">
        <div className="mb-6">
          ...
          <p className="text-lg font-normal text-monkblack">
            Below are your uploaded photos
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {images.map((image, index) => (
            <div
              key={index}
              className="relative border-2 border-gray-300 rounded-lg w-full h-[320px]"
            >
              <img
                src={image}
                alt="Uploaded"
          ...
          <div className="flex flex-col items-center justify-center border-2 border-gray-300 p-1 rounded-lg w-full h-[320px]">
            ...
            <p className="text-sm text-monkblack font-normal mb-2 text-center">
