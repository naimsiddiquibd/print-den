...
const ChooseSizes = () => {
    ...
    const handleContinue = () => {
        ...
        navigate('/personal-info', { state: { selectedImages: images, selectedSizes } });
    };
            ...
            <div className='bg-monkwhite py-10 pt-32 min-h-screen'>
                <div className='lg:w-[1180px] lg:mx-auto'>
                    <div className='mb-6'>
                        ...
                        <p className='text-lg font-normal text-monkblack'>Choose sizes for your photos</p>
                    </div>
                    <div className='grid grid-cols-1 gap-6'>
                        {/* Loop through selected images and display each with size options */}
                        {images && images.length > 0 ? (
                            images.map((image, index) => (
                                <div key={index} className='flex gap-4 bg-monkwhite shadow-sm p-4 rounded-lg relative'>
                                    <div className=''>
                                        <img className='rounded-md w-full h-60 object-cover' src={image} alt={`Selected Image ${index + 1}`} />
                                    </div>
                                    <div className='flex flex-col gap-3 mt-3'>
                                        {/* Size options for each image */}
                                        {sizeOptions.map((size) => (
                                            <div key={size.id} className='flex items-center justify-start gap-2'>
                                                <input
                                                    type="radio"
                                                    name={`size-${index}`}
                                                    className="radio"
