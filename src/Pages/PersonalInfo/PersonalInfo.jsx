...
const PersonalInfo = () => {
  ...
  const handleSubmit = async () => {
    ...
    selectedImages.forEach((image, index) => {
      ...
      formDataObj.append(`documents[${index}][frame_id]`, ""); // empty string will be considered as null
    });

    try {
      const response = await fetch(
        "https://adminprintden.fatmonk.studio/api/service/submit",
        {
          method: "POST",
          body: formDataObj,
        }
    ...
    <div>
      ...
      <div className="bg-monkwhite py-10 pt-32 min-h-screen">
        <div className="lg:w-[1180px] lg:mx-auto">
          <div className="mb-6">
            <p className="text-3xl font-bold text-monkblack">
              Personal Information
            </p>
            <div>
              <label className="form-control w-full">
                <div className="label">
                  <span className="label-text text-sm text-monkblack font-medium">
                    Name
                  </span>
                </div>
                <input
